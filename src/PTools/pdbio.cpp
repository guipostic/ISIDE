...
void ReadPDB(ifstream& fichier, Rigidbody& protein, char block) {
...
void ReadPDB(const std::string name,Rigidbody& protein, char block) {
    ...
    if (!fichier)
    {
        ...
        throw std::invalid_argument("\n* Error: ReadPDB cannot open file \"" + nomfich + "\"");
//        exit(-1);
    }

    ReadPDB(fichier, protein, block);
    fichier.close();
}
