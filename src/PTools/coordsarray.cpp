...
void CoordsArray::MatrixMultiply(const double mat[4][4])
{
...
std::string CoordsArray::PrintMatrix() const
{
    ...
    for (uint i=0; i<4; i++)
    {
        ...
        for (uint j=0; j<4; j++)
        {
            ...
            snprintf(tmp,19,"%14.7f ", this->mat44[i][j]);
            out += tmp;
        }
        out +="\n";
    }
...
//sets the coordinate [i] to be 'co' after rotation/translation
	...
	double matinv[4][4]; //invert matrix
...
*/
void CoordsArray::AttractEulerRotate(double phi, double ssi, double rot)
{
