...
/* ISIDE v1.0: the automatic derivative calculation (and the dbl type) have been removed */
...
{
    static inline double Pow( double x ) { return 1; }
};
...
public:
    ...
    bool almostEqual(const Array2D<mytype> & tocompare, double difference)
    {
        ...
        for (uint i=0; i<m_size; i++)
           if (  fabs(msa_data[i] - tocompare.msa_data[i]) > difference) return false;
